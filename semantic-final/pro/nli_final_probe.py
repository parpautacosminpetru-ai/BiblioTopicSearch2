...
for premise, hypothesis in pairs:
    forward = probabilities(premise, hypothesis)
    reverse = probabilities(hypothesis, premise)
    print("NLI_FWD", premise, "||", hypothesis, "=>", [round(float(x), 6) for x in forward])
    print("NLI_REV", hypothesis, "||", premise, "=>", [round(float(x), 6) for x in reverse])
